...
const initialData = {
  ...
  messages: {
    'c1': [
      // --- MAY 26 MORNING: AFTERMATH OF THE INCIDENT ---
      ...
      // --- MAY 26 MORNING: RUDH TEXTING ALONE ---
      ...
      // --- MAY 26 LATER: TEXTING INTO THE VOID ---
  ...
  return (
    <div className="flex h-screen w-full bg-[#1e1f22] text-[#dbdee1] font-sans overflow-hidden">
      ...
      <div className="w-[72px] bg-[#1e1f22] shrink-0 flex flex-col items-center py-3 z-20">
        ...
        <div className="mt-auto mb-2 flex flex-col items-center gap-2">
           ...
           <Tooltip text="Download Apps">
             <div className="group cursor-pointer w-12 h-12 hover:bg-[#313338] rounded-[24px] hover:rounded-[16px] transition-all duration-300 flex items-center justify-center text-[#23a559]">
                <Download size={24} />
             </div>
           </Tooltip>
        </div>
      ...
      <div className="flex-1 bg-[#313338] flex flex-col min-w-0">
        ...
        {/* Message History */}
        <div 
          className="flex-1 overflow-y-auto custom-scrollbar px-4 pt-4 pb-2"
          ...
          {channelMessages.map((msg, index) => {
            ...
            return (
              <React.Fragment key={msg.id}>
                ...
                <div 
                  className={`flex items-start group hover:bg-[#2e3035] -mx-4 px-4 py-0.5 relative ${isConsecutive ? 'mt-0' : 'mt-4'}`}
                  onContextMenu={(e) => {
                    e.preventDefault();
                ...
                >
                  ...
                  {isConsecutive ? (
                     <div className="w-10 mr-4 shrink-0 text-center opacity-0 group-hover:opacity-100 flex items-center justify-center pt-1">
                    ...
                    {editingMessageId === msg.id ? (
                      <div className="mt-1 mb-1 pr-12">
                          ...
                          <input
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                                setMessages(prev => ({
                                  ...prev,
                                  [activeChannelId]: prev[activeChannelId].map(m => m.id === msg.id ? { ...m, text: editMessageText } : m)
                                }));
                        ...
                        <div className="text-xs text-[#949ba4] mt-1">
                          escape to <span className="text-[#00a8fc] cursor-pointer hover:underline" onClick={() => setEditingMessageId(null)}>cancel</span> • enter to <span className="text-[#00a8fc] cursor-pointer hover:underline" onClick={() => {
                            setMessages(prev => ({
                              ...prev,
                              [activeChannelId]: prev[activeChannelId].map(m => m.id === msg.id ? { ...m, text: editMessageText } : m)
                            }));
                      ...
                      <div>
                        <p className="text-[#dbdee1] leading-relaxed break-words whitespace-pre-wrap">{msg.text}</p>
                        ...
                        {msg.image && (
                          <div className="mt-2 relative inline-block">
                            <img 
                              src={msg.image} 
                              alt="attachment" 
                              className="max-w-sm max-h-[350px] object-cover rounded-lg border border-[#1e1f22] cursor-pointer hover:opacity-95" 
                            />
                          </div>
                        )}
                      </div>
      ...
      {contextMenu && (
        ...
        >
          <div 
            className="px-3 py-1.5 hover:bg-[#4752c4] hover:text-white cursor-pointer flex items-center justify-between mx-1 rounded-sm group transition-colors"
            onClick={() => {
              setEditingMessageId(contextMenu.message.id);
              setEditMessageText(contextMenu.message.text);
              setContextMenu(null);
            }}
          >
          ...
          <div 
            className="px-3 py-1.5 hover:bg-[#da373c] text-[#f23f43] hover:text-white cursor-pointer flex items-center justify-between mx-1 rounded-sm group transition-colors"
            onClick={() => {
              setMessages(prev => ({
                ...prev,
                [activeChannelId]: prev[activeChannelId].filter(m => m.id !== contextMenu.message.id)
              }));
              setContextMenu(null);
            }}
          >
      ...
      {/* Basic Global Styles for Scrollbar */}
